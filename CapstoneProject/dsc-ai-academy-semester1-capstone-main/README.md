...
5. `.gitignore`
   ...
   * To tell Git to ignore more files, just add a new line to `.gitignore` for each new file name
     ...
     * The data folder `zippedData` has been added to `.gitignore` for you. If you find it necessary to include any of the data in your github repository, you can simply exclude that file from `.gitignore`. For example, if you wanted to include `im.db.zip` in your repository, you would add `!im.db.zip` to `.gitignore` and that file will be included in the remote repository when you push your changes.
